...
def on_company_change(i):
    """Runs BEFORE the page rerenders, so the address inputs receive the selected company's address."""
    selected = st.session_state.get(f"company_{i}", "")
    if selected in COMPANY_DATA:
        address = list(COMPANY_DATA[selected])[:3]
        address += [""] * (3 - len(address))
    else:
        address = ["", "", ""]
    st.session_state.companies[i]["name"] = selected
...
for i, row in enumerate(st.session_state.companies):
    ...
    current = row["name"] if row["name"] in COMPANY_DATA else ""
    company_key = f"company_{i}"

    # Initialize the selectbox state only once.
    ...
    st.selectbox(
        ...
        args=(i,),
        help="Click the box and type a few letters to search the company list.",
    )

    selected = st.session_state[company_key]
    if selected in COMPANY_DATA:
        st.success("✓ Address auto-filled below — you can edit it directly.")
    elif selected:
